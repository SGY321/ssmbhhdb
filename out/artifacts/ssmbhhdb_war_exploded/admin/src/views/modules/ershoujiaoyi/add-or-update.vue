<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="交易类型" prop="jiaoyileixing">
          <el-select v-model="ruleForm.jiaoyileixing" placeholder="请选择交易类型">
            <el-option
                v-for="(item,index) in jiaoyileixingOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input" label="交易类型" prop="jiaoyileixing">
	      <el-input v-model="ruleForm.jiaoyileixing"
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item class="textarea" v-if="type!='info'" label="交易详情" prop="jiaoyixiangqing">
                <el-input
                  style="min-width: 200px; max-width: 600px;"
                  type="textarea"
                  :rows="8"
                  placeholder="交易详情"
                  v-model="ruleForm.jiaoyixiangqing" >
                </el-input>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jiaoyixiangqing" label="交易详情" prop="jiaoyixiangqing">
                    <span>{{ruleForm.jiaoyixiangqing}}</span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
  ...
  methods: {
    ...
    init(id,type) {
      ...
      if(this.type=='info'||this.type=='else'){
	    ...
	    this.ro.wupinmingcheng = true;
          ...
          if(o=='jiaoyixiangqing'){
            this.ruleForm.jiaoyixiangqing = obj[o];
	    this.ro.jiaoyixiangqing = true;
            continue;
          }
          if(o=='jiaoyileixing'){
            this.ruleForm.jiaoyileixing = obj[o];
	    this.ro.jiaoyileixing = true;
            continue;
          }
          if(o=='jiage'){
      ...
      this.$http({
        ...
        if (data && data.code === 0) {
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/jiaoyileixing/jiaoyileixing`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.jiaoyileixingOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `ershoujiaoyi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.ershoujiaoyiCrossAddOrUpdateFlag = false;
                  this.parent.search();
